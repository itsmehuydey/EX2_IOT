...
constexpr float CO_THRESHOLD = 30.0;         // ppm, giới hạn an toàn nghiêm ngặt hơn cho nhà máy
...
constexpr float FIRE_MQ2_THRESHOLD = 3000.0; // Giá trị ADC, khí dễ cháy nguy hiểm
constexpr float FIRE_TEMP_THRESHOLD = 60.0;  // °C, nhiệt độ cao bất thường
...
void task_SendTelemetry()
{
  ...
  // Đọc các giá trị khí
  MQ135.setA(605.18);
  MQ135.setB(-3.937); // Cấu hình cho CO
  float CO = MQ135.readSensor();

  MQ135.setA(77.255);
  MQ135.setB(-3.18); // Cấu hình cho Alcohol
  float Alcohol = MQ135.readSensor();

  MQ135.setA(110.47);
  MQ135.setB(-2.862); // Cấu hình cho CO2
  float CO2 = 410 + MQ135.readSensor();

  MQ135.setA(44.947);
  MQ135.setB(-3.445); // Cấu hình cho Toluene
  float Toluene = MQ135.readSensor();

  MQ135.setA(102.2);
  MQ135.setB(-2.473); // Cấu hình cho NH4
  float NH4 = MQ135.readSensor();

  MQ135.setA(34.668);
  MQ135.setB(-3.369); // Cấu hình cho Acetone
  ...
  // Tính và gửi thông báo cháy
  bool fireAlert = (mq2Value > FIRE_MQ2_THRESHOLD || temperature > FIRE_TEMP_THRESHOLD);
  String fireStatus = fireAlert ? "Fire Detected" : "No Fire";
  Serial.print("Fire Status: ");
  Serial.println(fireStatus);
  tb.sendTelemetryData("fire_alert", fireStatus.c_str());
...
void task_UpdateLCD()
{
  ...
  else
  {
    ...
    {
      lcd.clear();
      currentDisplay = (currentDisplay == DHT_20) ? Light : (currentDisplay == Light) ? Alert
                                                                                      : DHT_20;
      lastDisplaySwitch = millis();
    }
...
void setup()
{
  ...
  rgbLed.begin();
  rgbLed.setBrightness(50); // Độ sáng 50% (0-255), điều chỉnh nếu cần
  rgbLed.clear();
  rgbLed.show(); // Tắt LED ban đầu
